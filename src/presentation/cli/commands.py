...
Examples:
  ...
  python main.py info --office-id 1 --date 2025-12-03 --start-time 10:00 --end-time 12:00
        ...
        check_parser = subparsers.add_parser("check-availability", help="Check if an office is available")
        ...
        book_parser.add_argument("--phone", required=True, help="Your phone number (international format, e.g., +1234567890)")
    ...
    def _add_common_args(parser: argparse.ArgumentParser) -> None:
        parser.add_argument("--office-id", type=int, required=True, choices=[1, 2, 3, 4, 5], help="Office ID (1-5)")
    ...
    def _handle_info(self, args: argparse.Namespace) -> int:
        ...
        if result["occupied"]:
            print(f"[OCCUPIED] Office is OCCUPIED\n")
            print(f"Office: {result['data']['office_name']} (#{result['data']['office_id']})")
            print(f"Occupied by: {result['data']['occupied_by']}")
            print(f"Contact: {result['data']['occupant_email']}, {result['data']['occupant_phone']}")

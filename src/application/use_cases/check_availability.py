...
class CheckAvailabilityUseCase:
    ...
    def execute(self, office_id: int, time_slot: TimeSlot) -> AvailabilityDTO:
        ...
        for reservation in conflicting_reservations:
            conflicts.append(
                ConflictingReservationDTO(
                    ...
                    start_time=reservation.time_slot.start_time.strftime(
                        "%Y-%m-%d %H:%M"
                    ),
                    end_time=reservation.time_slot.end_time.strftime(
                        "%Y-%m-%d %H:%M"
                    ),
                )
            )
        ...
        return AvailabilityDTO(
            ...
            requested_start_time=time_slot.start_time.strftime(
                "%Y-%m-%d %H:%M"
            ),

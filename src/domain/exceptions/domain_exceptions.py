...
class InvalidTimeSlotError(DomainException):
    def __init__(self, message: str = "Invalid time slot", **kwargs) -> None:
        super().__init__(message, kwargs)


class OfficeNotAvailableError(DomainException):
    def __init__(
        self,
        office_id: int,
...
class OfficeNotFoundError(DomainException):
    def __init__(self, office_id: int) -> None:
        super().__init__(
            f"Office with ID {office_id} not found",
            {"office_id": office_id}
        )


class ReservationConflictError(DomainException):
    def __init__(
        self,
        office_id: int,
        ...
        **kwargs
        ...
        message = (
            f"Office {office_id} is occupied by {conflicting_user} "
            f"until {end_time}"
        )
        details = {
            "office_id": office_id,

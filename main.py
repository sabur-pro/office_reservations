...
from src.infrastructure.database.repositories.office_repository import (
    OfficeRepository,
)
...
from src.presentation.controllers.reservation_controller import (
...
def initialize_database(db: DatabaseConnection) -> None:
    ...
    with db.session_scope() as session:
        ...
        offices = [
            Office(office_id=1, name="Conference Room A", capacity=10, description="Large conference room with projector"),
            Office(office_id=2, name="Meeting Room B", capacity=6, description="Medium meeting room with whiteboard"),
            Office(office_id=3, name="Small Meeting Room C", capacity=4, description="Small room for focused discussions"),
            Office(office_id=4, name="Executive Suite", capacity=8, description="Premium meeting space with video conferencing"),
            Office(office_id=5, name="Collaboration Space", capacity=12, description="Open collaboration area with multiple zones"),
        ]
...
def create_dependency_container(db: DatabaseConnection) -> ReservationController:
    ...
    sms_notifier = SMSNotifier(api_key=settings.sms_api_key, sender_number=settings.sms_sender_number)
    notification_service = CombinedNotificationService(email_notifier=email_notifier, sms_notifier=sms_notifier)
...
def main() -> int:
    ...
    controller = create_dependency_container(db)
    cli = CLI(controller)
    return cli.run()
